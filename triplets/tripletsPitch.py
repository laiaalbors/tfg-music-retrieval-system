...
import torch
import pandas as pd
from datasets import TripletPitch
...
#-------------------------------------------------#
train = pd.read_csv("../datasetPitches/train/TRAIN.csv", header=0)
valid = pd.read_csv("../datasetPitches/validation/VALIDATION.csv", header=0)
...
#-------------------------------------------------#
triplet_train_dataset = TripletPitch(partition['train'], labels, train=True) # Returns triplets of arrays
triplet_valid_dataset = TripletPitch(partition['validation'], labels, train=False)
...
from networks import EmbeddingNetPitch, TripletNet
...
embedding_net = EmbeddingNetPitch(hidden_dim, output_dim, kernel_size, nhid, embedding_size, dropout)
model = TripletNet(embedding_net)
improve = False
if pth.isfile("./exp-"+str(experiment)+"/modelPitch-"+str(experiment)+".pt"):
    model.load_state_dict(torch.load("./exp-"+str(experiment)+"/modelPitch-"+str(experiment)+".pt"))
    improve = True
if cuda:
    model.cuda()
...
#-------------------------------------------------#
if improve:
    torch.save(model.state_dict(), "./exp-"+str(experiment)+"/modelPitch2-"+str(experiment)+".pt")
else:
    torch.save(model.state_dict(), "./exp-"+str(experiment)+"/modelPitch-"+str(experiment)+".pt")
