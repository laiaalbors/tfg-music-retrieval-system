...
output_dim = int(arg[6])
kernel_size = int(arg[7])
embedding_size = int(arg[8])
experiment = int(arg[9])
print("----------------")
print("PARAMETERS:")
...
#-------------------------------------------------#
train = pd.read_csv("../vggishBig/train/TRAIN1.csv", header=0)
valid = pd.read_csv("../vggishBig/validation/VALIDATION.csv", header=0)
...
from networks import EmbeddingNetVGGishSmall, TripletNet
...
embedding_net = EmbeddingNetVGGishSmall(output_dim, kernel_size, embedding_size, dropout)
model = TripletNet(embedding_net)
improve = False
if pth.isfile("./experimentsVGGishSmallBig/exp-"+str(experiment)+"/modelVGGishSmallBig-"+str(experiment)+".pt"):
    model.load_state_dict(torch.load("./experimentsVGGishSmallBig/exp-"+str(experiment)+"/modelVGGishSmallBig-"+str(experiment)+".pt"))
    improve = True
if cuda:
    model.cuda()
...
#-------------------------------------------------#
if improve:
    torch.save(model.state_dict(), "./experimentsVGGishSmallBig/exp-"+str(experiment)+"/modelVGGishSmallBig2-"+str(experiment)+".pt")
else:
    torch.save(model.state_dict(), "./experimentsVGGishSmallBig/exp-"+str(experiment)+"/modelVGGishSmallBig-"+str(experiment)+".pt")
