...
import torch
import pandas as pd
from datasets import TripletSegDTW
...
path = "./experiments/exp-" + str(experiment) + "/"
...
#-------------------------------------------------#
triplet_train_dataset = TripletSegDTW(partition['train'], labels, alignment, train, train=True) # Returns triplets of arrays
triplet_valid_dataset = TripletSegDTW(partition['validation'], labels, alignment, valid, train=False)
...
if pth.isfile("./experiments/exp-"+str(experiment)+"/modelSegDTW2-"+str(experiment)+".pt"):
    model.load_state_dict(torch.load("./experiments/exp-"+str(experiment)+"/modelSegDTW2-"+str(experiment)+".pt"))
else:
...
#-------------------------------------------------#
torch.save(model.state_dict(), "./experiments/exp-"+str(experiment)+"/modelSegDTW3-"+str(experiment)+".pt")
print("Model guardat.")
