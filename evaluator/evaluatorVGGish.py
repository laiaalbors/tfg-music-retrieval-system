...
def plot_embeddings(embeddings, targets, name, xlim=None, ylim=None):
    ...
    plt.legend(classes)
    if improve:
        plt.savefig("./exp-"+str(experiment)+'/'+name+"VGGish2-"+str(experiment)+'.png')
    else:
        plt.savefig("./exp-"+str(experiment)+'/'+name+"VGGish-"+str(experiment)+'.png')
...
if improve:
    ...
    model.load_state_dict(torch.load("./exp-"+str(experiment)+"/modelVGGish2-"+str(experiment)+".pt"))
else:
