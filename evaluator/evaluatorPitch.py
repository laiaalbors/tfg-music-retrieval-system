...
from datasets import SingularPitch
...
#-------------------------------------------------#
train_dataset = SingularPitch(partition['train'], labels, train=True)
valid_dataset = SingularPitch(partition['validation'], labels, train=False)
...
def plot_embeddings(embeddings, targets, name, xlim=None, ylim=None):
    ...
    plt.legend(classes)
    if improve:
        plt.savefig("./exp-"+str(experiment)+'/'+name+"Pitch2-"+str(experiment)+'.png')
    else:
        plt.savefig("./exp-"+str(experiment)+'/'+name+"Pitch-"+str(experiment)+'.png')
...
if improve:
    ...
    model.load_state_dict(torch.load("./exp-"+str(experiment)+"/modelPitch2-"+str(experiment)+".pt"))
else:
