...
from datasets import SingularVGGishMean
...
experiment = int(arg[3])
improve = bool(int(arg[4]))
print("----------------")
print("PARAMETERS:")
print("k =", k)
...
#-------------------------------------------------#
train_dataset = SingularVGGishMean(partition['train'], labels, train=True)
valid_dataset = SingularVGGishMean(partition['validation'], labels, train=False)
...
def plot_embeddings(embeddings, targets, name, xlim=None, ylim=None):
    ...
    plt.legend(classes)
    if improve:
        plt.savefig("./experimentsVGGishSmallMean/exp-"+str(experiment)+'/'+name+"VGGishSmallMean2-"+str(experiment)+'.png')
    else:
        plt.savefig("./experimentsVGGishSmallMean/exp-"+str(experiment)+'/'+name+"VGGishSmallMean-"+str(experiment)+'.png')
...
embedding_net = EmbeddingNetVGGishSmallMean(embedding_size)
model = TripletNet(embedding_net)
if improve:
    print("   Taking the improved model.")
    model.load_state_dict(torch.load("./experimentsVGGishSmallMean/exp-"+str(experiment)+"/modelVGGishSmallMean2-"+str(experiment)+".pt"))
else:
    model.load_state_dict(torch.load("./experimentsVGGishSmallMean/exp-"+str(experiment)+"/modelVGGishSmallMean-"+str(experiment)+".pt"))
if cuda:
    model.cuda()
#model.eval()
