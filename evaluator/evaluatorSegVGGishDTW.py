...
def plot_embeddings(embeddings, targets, name, xlim=None, ylim=None):
    ...
    plt.legend(classes)
    plt.savefig("./experimentsVGGish/exp-"+str(experiment)+'/'+name+"SegVGGishDTW-"+str(experiment)+'.png')
...
model.load_state_dict(torch.load("./experimentsVGGish/exp-"+str(experiment)+"/modelSegVGGishDTW-"+str(experiment)+".pt", map_location=torch.device('cpu')))
