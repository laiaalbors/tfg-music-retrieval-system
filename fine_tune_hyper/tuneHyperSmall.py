...
def train_triplets(config, checkpoint_dir=None, data_dir=None):
    embedding_net = EmbeddingNetVGGishSmall(config['output_dim'],
                                            config['kernel_size'],
...
def main(num_samples=10, max_num_epochs=30, gpus_per_trial=2):
    ...
    config = {
        ...
        "output_dim": tune.choice([8,16, 32, 64]),
        "kernel_size": 3,
        "embedding_size": tune.choice([128, 256, 512,1024]),
        "k": 100
    }
    ...
    embedding_net = EmbeddingNetVGGishSmall(best_trial.config['output_dim'],
                                            best_trial.config['kernel_size'],
                                            best_trial.config['embedding_size'],
