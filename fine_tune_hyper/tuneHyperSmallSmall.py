...
from networks import EmbeddingNetVGGishSmallSmall, TripletNet
    ...
    with torch.no_grad():
        ...
        for images, target in dataloader:
            ...
            if size == 2:
                ...
                embeddings[k:k+len(images)] = model.get_embeddingSmall(images).data.cpu().numpy()
...
def train_triplets(config, checkpoint_dir=None, data_dir=None):
    embedding_net = EmbeddingNetVGGishSmallSmall(config['embedding_size'])
...
def main(num_samples=10, max_num_epochs=30, gpus_per_trial=2):
    ...
    embedding_net = EmbeddingNetVGGishSmallSmall(best_trial.config['embedding_size'])
