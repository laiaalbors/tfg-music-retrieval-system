...
import pandas as pd
from datasets import TripletVGGish, SingularVGGish
...
def load_data(data_dir="../vggish", tr=True):
    ...
    if tr:
        triplet_train_dataset = TripletVGGish(partition['train'], labels, train=True)
        triplet_valid_dataset = TripletVGGish(partition['validation'], labels, train=False)
    else:
        triplet_train_dataset = SingularVGGish(partition['train'], labels, train=True)
        triplet_valid_dataset = SingularVGGish(partition['validation'], labels, train=False)
...
def train_triplets(config, checkpoint_dir=None, data_dir=None):
    embedding_net = EmbeddingNetVGGish(config['hidden_dim'], config['output_dim'],
                                       config['kernel_size'], config['nhid'],
                                       config['embedding_size'], config['dropout'])
...
def main(num_samples=10, max_num_epochs=30, gpus_per_trial=2):
    ...
    config = {
        ...
        "output_dim": tune.choice([16, 32, 64]),
        "kernel_size": 3,
        "nhid": tune.choice([2048, 1024, 512, 256]),
    ...
    embedding_net = EmbeddingNetVGGish(best_trial.config['hidden_dim'], best_trial.config['output_dim'],
                                       best_trial.config['kernel_size'], best_trial.config['nhid'],
                                       best_trial.config['embedding_size'], best_trial.config['dropout'])
...
if __name__ == "__main__":
    ...
    main(num_samples=30, max_num_epochs=30, gpus_per_trial=2)
